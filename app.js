...
function applyUserUI(){
  /* أظهر واجهة النظام كاملة فقط بعد ما المستخدم يتسجل دخوله بنجاح */
...
function showLock(){
  /* أخفي واجهة النظام كاملة قبل ما قفل الدخول يبان — عشان مايحصلش فلاش لمحتوى البرنامج */
...
async function factoryReset(){
  ...
  await ensureFirebaseAdminAuth(okPass); /* لازم الهوية الحقيقية قبل أي مسح فعلي */
  // 1) امسح السيرفر بس — لكن **احفظ بيانات الربط** (meta.config) عشان مايضطرش يربط من أول وجديد
  async function wipeServer(){
    /* نمسح كل العقد ما عدا /meta/config (بيانات الربط) */
    const topNodes = ['items','notifs','sessions','auditLog'];
    for (const n of topNodes){ try { await db.ref(fbRoot() + '/' + n).remove(); } catch(e){} }
    /* نمسح meta كلها ما عدا config والختم configRev */
    const metaSnap = await db.ref(fbRoot() + '/meta').get();
    const metaVal = metaSnap.val() || {};
    const keepConfig = metaVal.config || null;
    const keepConfigRev = metaVal.configRev || null;
    await db.ref(fbRoot() + '/meta').remove();
    if (keepConfig) await db.ref(fbRoot() + '/meta/config').set(keepConfig);
    if (keepConfigRev) await db.ref(fbRoot() + '/meta/configRev').set(keepConfigRev);
    return true;
  }
  ...
  if (!serverOk) {
    // جرب REST مباشر مع فحص status فعلي — نفس المبدأ: نمسح عقد محددة بدل الجذر كله
    try {
      const cfg = effectiveCfg();
      if (cfg && cfg.databaseURL) {
        const base = cfg.databaseURL.replace(/\/$/, '');
        const tok = '?auth=' + encodeURIComponent(await getAnonIdToken(cfg));
        const nodes = ['items','notifs','sessions','auditLog','meta'];
        let allOk = true;
        for (const n of nodes){
          const r = await fetch(base + '/' + fbRoot() + '/' + n + '.json' + tok, { method: 'DELETE' });
          if (!r.ok) allOk = false;
        }
        serverOk = allOk;
        if (!serverOk) failReason = 'HTTP error';
      } else if (!failReason) {
        failReason = 'مفيش اتصال بقاعدة البيانات دلوقتي';
      }
    } catch(e){ failReason = e && e.message ? e.message : String(e); }
  }
  // 2) امسح كل حاجة محلية — لكن **احفظ إعدادات الربط** (firebaseCfg) عشان مايرجعش شاشة اللصق من أول
  let savedCfg = null;
...
async function openCameraScanner(){
  ...
  try {
    ...
    qrCamOn = true;
    /* 🎯 سر التقاط الباركود الصغير: رفع الدقة وفوكس مستمر — بس بدون زوم */
    ...
    /* زر الفلاش — يظهر دائمًا لو الكاميرا بتدعمه، وبتقدر تشغّله وتطفّيه في أي وقت */
    try {
