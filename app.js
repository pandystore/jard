...
async function factoryReset(){
  ...
  try {
    if (syncOn && db) await db.ref(fbRoot() + '/meta/forceWipe').set(firebase.database.ServerValue.TIMESTAMP);
  } catch(e){}
  // 2) امسح كل حاجة محلية — لكن **احفظ إعدادات الربط** (firebaseCfg) وآخر forceWipe عشان مايعملش reload تاني بعد المسح
  let savedCfg = null, lastWipeStamp = '';
  try { savedCfg = store.getItem('firebaseCfg'); } catch(e){}
  try { lastWipeStamp = String(Date.now()); } catch(e){}
  try {
    inventoryData = [];
    usersList = [];
    adminHash = '';
    ...
    store.setItem('lastForceWipe', lastWipeStamp);
